...
import ChatLevel from "@/components/ChatLevel";
...
const Sidebar = ({expand,setExpand}) => {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transiton-all z-50 max-md:absolute max-md:h-screen ${expand?'p-4 w-64':'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        <div className= {`flex ${expand?"flex-row gap-10":"flex-col items-center gap-8"} `}>
          ...
          className='group relative flex items-center justify-center hover:bg-gray-500/20 tansition-all duration-300 h-9 w-9 aspect-squere rounded-lg curser-pointer'>
            <Image src={assets.menu_icon} alt='' className='md:hidden'/>
            <Image src={expand?assets.sidebar_close_icon:assets.sidebar_icon} alt='hidden md:block w-7'/>
            <div className={`absolute w-max ${expand ? "let-1/2 -translate-x-1/2 top-12" : "-top-12 left-0"} opacity-0 group-hover:opacity-100 translation bg-black text-white text-sm px-3 py-2 rounded-lg pointer-events-none`}>
        ...
        <button onClick={createNewChat} className={`mt-8 flex item-center justify-center cursor-pointer
          ...
          <div className='absolute w-max -top-12 -right-12 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
            New chat
            <div className='h-3 w-3 absolute bg-black roate-45 left-4 -bottom-1.5'></div>
          </div>
          {expand && <p className='text-white text text-medium'>New chat</p>}
        </button>
        <div className={`mt-8 text-white/50 text-sm ${expand?"block":"hidden"}`}>
          <p className='my-1'>Recents</p>
          {(chats ||[]).map((chat, index)=> <ChatLevel key={index} name={chat.name} id={chat._id} openMenu={openMenu} setOpenMenu={setOpenMenu} />)}
      ...
      <div className={`flex items-center cursor-pointer group relative ${expand?"gap-1 text-white/80 text-sm p-2.5 border border-primary rounded-lg hover:bg-white/10 cursor-pointer ":"h-10 w-10 mx-auto hover:bg-gray-500/30 rounded-lg"}`} >
        ...
        <div className={`absolute -top-60 pb-8 ${!expand && "-right-40"} opacity-0 group-hover:opacity-100 hidden group-hover:block transition`}>
          <div className='relative w-max bg-black text-white text-sm p-3 rounded-lg shadow-lg'>
            ...
            <div className={`h-3 w-3 absolute bg-black rotate-45 ${expand?"left-1/2":"right-4"} -bottom-1.5`}></div>
          </div>
        </div>
